...
ggplot(aes(x = Year, y = gpp )) + geom_line(color = '#00868B', alpha = 0.6) + geom_point_interactive(aes(data_id = gpp, tooltip = gpp), color = '#00868B') +
  ...
  labs(x = "", y = "Plastik in Millionen Tonnen", title = "Im Vergleich zu 1950 hat sich 2015 die Plastikproduktion in\n Millionen Tonnen fast um den Faktor 200 erhöht", subtitle = "Globale Entwicklung der Plastikproduktion von 1950 bis 2015") +
...
mydata2 %>%
  ggplot(aes(x = `Per capita plastic waste (g/person/day)`, y = `Per capita mismanaged plastic waste`, size = BIP, fill = Kontinent)) + geom_point_interactive(position = "jitter", shape = 21, color = "black", aes(data_id = Entity, tooltip = Entity)) +                                                 scale_x_continuous(limits = c(0, 700), breaks = c(0, 100, 200, 300, 400, 500, 600, 700)) +
  scale_y_continuous(limits = c(0, 300), breaks = c(0, 50, 100, 150, 200, 250, 300)) +
  theme_minimal_grid() +
  theme(axis.text.y = element_text(vjust = 0)) +
  scale_fill_brewer(palette = "Set1") +
  labs(x = "Plastikmüllproduktion pro Kopf (täglich in g)", y = "Inadäquat entsorgter Plastikmüll pro Kopf (täglich in g)", title = "kommt noch", subtitle = "Zusammenhang von Wohlstand, Plastikmüllproduktion und inadäquater Entsorgung") -> plot_per_capita_waste_mismanagement_GDP
